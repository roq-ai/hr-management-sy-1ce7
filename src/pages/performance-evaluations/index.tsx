...
export function PerformanceEvaluationListPage(props: PerformanceEvaluationListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getPerformanceEvaluations({
        ...
        order: params.order,
        searchTermKeys: ['evaluation.contains'],
        ...(params.filters || {}),
      }),
  ...
  const columns: ColumnType[] = [
    { id: 'evaluation', header: 'Evaluation', accessorKey: 'evaluation' },
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'user_performance_evaluation_employee_idTouser',
          header: 'User Performance Evaluation Employee Id Touser',
          accessorKey: 'user_performance_evaluation_employee_idTouser',
          cell: ({ row: { original: record } }: any) => (
            <Link
              as={NextLink}
              onClick={(e) => e.stopPropagation()}
              href={`/users/view/${record.user_performance_evaluation_employee_idTouser?.id}`}
            >
              {record.user_performance_evaluation_employee_idTouser?.email}
            </Link>
          ),
        }
      : null,
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'user_performance_evaluation_team_lead_idTouser',
          header: 'User Performance Evaluation Team Lead Id Touser',
          accessorKey: 'user_performance_evaluation_team_lead_idTouser',
          cell: ({ row: { original: record } }: any) => (
            <Link
              as={NextLink}
              onClick={(e) => e.stopPropagation()}
              href={`/users/view/${record.user_performance_evaluation_team_lead_idTouser?.id}`}
            >
              {record.user_performance_evaluation_team_lead_idTouser?.email}
            </Link>
          ),
        }
      : null,
